...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const transfer_parsed = chain
      ...
      .result.expectOk()
      .expectTuple();
    const deploy_parsed = chain.callReadOnlyFn('clarity-bitcoin', 'parse-wtx', [deploy_data], deployer.address).result.expectOk().expectTuple();
    const inscription_encoded = deploy_parsed.witnesses.expectList()[0].expectList()[1];

    console.log(inscription_encoded.slice(158,inscription_encoded.length - 2));
